...
6. Изменение размера изображения для стикера: Бот изменяет размер изображения, сохраняя пропорции, чтобы его 
максимальное измерение не превышало 512 пикселей, что подходит для использования в качестве стикера в Telegram.
7. Случайная шутка: Бот отправляет случайную шутку из списка шуток при получении команды /random_joke.
8. Случайный комплимент: Бот отправляет случайный комплимент из списка комплиментов при получении команды 
/random_compliment.
9. **Подбрасывание монетки**: Бот случайным образом выбирает результат ("Орел" или "Решка") при получении команды 
`/flip_coin`.
...
5. **Получите случайную шутку:** Отправьте команду /random_joke, чтобы получить случайную шутку.
6. **Получите случайный комплимент:** Отправьте команду /random_compliment, чтобы получить случайный комплимент.
7. **Подбросьте монетку:** Отправьте команду /flip_coin, чтобы бот случайным образом выбрал результат ("Орел" или 
"Решка").
...
Пример изображения, адаптированного для использования в качестве стикера:

<img src="cat.jpg" width="200" height="200" />

**Случайная шутка**

Пример случайной шутки, которую может отправить бот:
...
Пример случайного комплимента, который может отправить бот:
...
Пример результата подбрасывания монетки:

```text
Результат подбрасывания монетки: Орел
```
...
- **random:** Для выбора случайной шутки и случайного комплимента.
